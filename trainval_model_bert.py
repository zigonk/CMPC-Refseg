...
from util import data_reader_refvos_bert, data_reader
...
def train(max_iter, snapshot, dataset, data_dir, setname, mu, lr, bs, tfmodel_folder,
    ...
    im_h, im_w, num_steps, bert_size = model.H, model.W, model.num_steps, model.bert_size
...
def test(iter, dataset, visualize, setname, dcrf, mu, tfmodel_folder, model_name, pre_emb=False):
    ...
    for n_iter in range(reader.num_batch):
        ...
        scores_val, up_val, sigm_val, words_parse = sess.run([model.pred, model.up, model.sigm, model.words_parse],
                                                feed_dict={
        ...
        print('Sentence:', sent)
        print('Words type:', words_parse[0][0])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-meta', type=str, default='./train_metadata.json')
