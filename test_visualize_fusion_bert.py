...
#             Process text
            ve_id = vid + '_' + eid
            text_preprocessing = np.load(os.path.join(args.bert, ve_id + '.npz'))
...
#                 mask = np.array(frame, dtype=np.float32)
                ...
                scores_val, up_val, sigm_val, up_c3, up_c4, up_c5, words_parse = sess.run([model.pred, 
                                                                                                ...
                                                                                                feed_dict={
                                                                                                    model.words_feat: np.expand_dims(text_preprocessing['feature'], axis=0),
                                                                                                    model.im: np.expand_dims(proc_im_, axis=0),
                                                                                                    model.sequence_mask: np.expand_dims(text_preprocessing['mask'], axis=0)
                                                                                                })
