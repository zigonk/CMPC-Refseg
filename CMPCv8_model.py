...
#         visual_feat_c3 = tf.nn.l2_normalize(visual_feat_c3, 3)
        ...
        words_parse, words_weight = self.build_lang_parser(words_feat)
...
#         self.up_c3 = tf.image.resize_bilinear(score_c3, [self.H, self.W])
        ...
        valid_lang = self.valid_lang(words_parse, words_feat)
#         fused_feats = self.gated_exchange_fusion_lstm_2times(fusion_c3,
#                                                              fusion_c4, fusion_c5, valid_lang)
        fusion_lang = self.valid_lang(words_weight, words_feat)
        fused_feats = self.gated_exchange_fusion_lstm_2times(fusion_c4, fusion_c5, valid_lang, fusion_lang, spatial)
...
#         feat2 = self.lang_se(feat2, gv_lang, level + '_f2')
    ...
    def gated_exchange_fusion_lstm_2times(self, feat4, feat5, lang_feat, fusion_lang_feat, spatial):
        '''
...
#         feat_exg3_2 = tf.nn.l2_normalize(feat_exg3_2, 3)
    ...
    def mutan_head(self, lang_feat, spatial_feat, visual_feat, in_channel, out_channel, level=''):
        ...
        lang_trans = self._conv("lang_trans_{}".format(level), lang_feat,
                                1, self.rnn_size, out_channel, [1, 1, 1, 1])
    ...
    def build_lang_parser(self, words_feat):
        ...
        words_parse = self._conv("words_parse_2", words_parse, 1, 500, 2, [1, 1, 1, 1])
        ...
        return self.words_parse[:, :, :, 0], self.words_parse[:, :, :, 1]
