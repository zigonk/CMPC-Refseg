...
def run_prefetch(prefetch_queue, folder_name, prefix, num_batch, shuffle, id2name):
    ...
    while True:
        ...
        if id2name:
            batch['img_name'] = id2name[str(batch_id)]
        npz_filemap.close()
...
class DataReader:
    def __init__(self, folder_name, prefix, shuffle=True, prefetch_num=8, list_name=''):
        ...
        self.id2name = None
        if list_name != '':
            img_list = [line.strip() for line in open(list_name)]
            self.id2name = {line.split('\t')[0]:line.split('\t')[-1] for line in img_list}
        ...
        self.prefetch_thread = threading.Thread(target=run_prefetch,
            args=(self.prefetch_queue, self.folder_name, self.prefix,
                  self.num_batch, self.shuffle, self.id2name))
